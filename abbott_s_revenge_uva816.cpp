...
int dir_id(char c) { return strchr(dirs, c) - dirs; } // 进去的朝向
int turn_id(char c) { return strchr(turns, c) - turns; } // 可以转的方向
//用于转弯。 
...
void solve() {
  ...
  while(!q.empty()) {
    ...
    for(int i = 0; i < 3; i++) {// 所有可能的转向，（直行，逆时针转， 顺时针转） 
      ...
      if(has_edge[u.r][u.c][u.dir][i] // 从这一步是否可以达到下一步
        && inside(v.r, v.c)  // 下一步是否出界，
        && d[v.r][v.c][v.dir] < 0)  // 下一步是否被走过（同方向）。
        {
